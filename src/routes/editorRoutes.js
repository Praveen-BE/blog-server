...
const isAdminOrCreator = (req, res, next) => {
  const userRole = req.user.role; // Populated from your database/JWT
  if (userRole === "admin" || userRole === "creator") {
    next(); // Permission granted
  } else {
    res
      .status(403)
      .json({ error: "Access denied. You do not have permission to post." });
  }
};
...
//   }
// });
editorRouter.get("/edit/:id", userAuth, isAdminOrCreator, async (req, res) => {
  try {
    const { id } = req.params;
    // const result = await req.app.locals.pool.query(
    //   `SELECT
...
editorRouter.post("/seosave", userAuth, isAdminOrCreator, async (req, res) => {
